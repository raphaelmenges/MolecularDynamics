...
ShaderProgram *ShaderProgram::texture(string name, Texture *texture)
{
    return this->texture(name, texture, 0);
}

ShaderProgram *ShaderProgram::texture(string name, Texture *texture, GLuint samplerHandle)
{
    Info* updateInfo = checkUpdate(name, "sampler2D");
    if (updateInfo != NULL) {
        TextureObject o;
        o.textureHandle = texture->getHandle();
        o.samplerHandle = samplerHandle;
        o.name = name;
        o.texture = texture;
        for (int i = 0; i < textureList.size(); i++) {
